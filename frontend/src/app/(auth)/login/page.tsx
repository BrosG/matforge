...
function LoginForm() {
  ...
  // Phone — always-visible direct input, no tab toggle.
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (!auth || !recaptchaRef.current) {
        setError("Phone sign-in is not configured.");
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      setError(humanizeAuthError(err, "Invalid code. Please try again."));
    } finally {
      setPhoneLoading(false);
    }
  };
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-8 py-12">
        ...
        >
          ...
          <div className="flex items-center gap-3 my-6">
            ...
            <span className="text-xs text-gray-600 uppercase tracking-wider">or sign in with phone</span>
            <div className="flex-1 h-px bg-gray-800" />
          </div>
